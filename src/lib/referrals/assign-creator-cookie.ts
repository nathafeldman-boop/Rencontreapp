 ...
 * signup) unless the visitor already carries a referral, creator, or
 * affiliate cookie from an earlier visit — first touch wins rather than
 * last click.
 */
export function assignCreatorCookie(request: NextRequest, response: NextResponse) {
  ...
  if (
    ...
    request.cookies.get(CREATOR_COOKIE) ||
